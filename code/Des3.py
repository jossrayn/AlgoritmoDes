class DesFinal:
    ...
    def fila_columna(self,cadena):
        for i in range(len( self.listaConversiones)):
            if int(cadena)== self.listaConversiones[i]:
                return i+1
            elif int(cadena)==0:
                return 0

    def permutacionSexteto(self,cadena,cajaS):
        fila = self.fila_columna(cadena[0]+cadena[5])
        columna = self.fila_columna(cadena[1:5])
        return self.convertir4Bits(str( self.listaConversiones[(cajaS[fila][columna])-1]))

    def convertir4Bits(self,cadena):
        tamano=len(cadena)
        for i in range(4):
            if tamano==4:
                return cadena
    ...
    def ultimoPaso(self,cadena):
        ...
        st0 = self.permutacionSexteto(cadena[0:6],s1)
        st1 = self.permutacionSexteto(cadena[6:12],s2)
        st2 = self.permutacionSexteto(cadena[12:18],s3)
        st3 = self.permutacionSexteto(cadena[18:24],s4)
        st4 = self.permutacionSexteto(cadena[24:30],s5)
        st5 = self.permutacionSexteto(cadena[30:36],s6)
        st6 = self.permutacionSexteto(cadena[36:42],s7)
        st7 = self.permutacionSexteto(cadena[42:48],s8)

        return st0+st1+st2+st3+st4+st5+st6+st7

    def permutacion(self,bloque):
